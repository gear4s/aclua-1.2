...
struct servermaprot : serverconfigfile
{
    ...
    virtual configset *get(int ccs);
    virtual int get_next(bool fromLua = false);
};
int servermaprot::get_next(bool fromLua)
{
  int ccs = curcfgset;
...
configset* servermaprot::get( int ccs ) { return configsets.inrange(ccs) ? &configsets[ccs] : NULL; }

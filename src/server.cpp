...
void serverdamage(client *target, client *actor, int damage, int gun, bool gib, const vec &hitpush = vec(0, 0, 0), bool fromLua = false)
{
    ...
    if(ts.health<=0)
    {
        if ( !fromLua ) if ( Lua::callHandler( LUA_ON_PLAYER_DEATH, "iibi", target->clientnum, actor->clientnum, gib, gun ) == Lua::PLUGIN_BLOCK ) return;
...
struct voteinfo
{
    ...
    void end(int result, bool fromLua = false)
    {
        ...
        if ( !fromLua && Lua::callHandler( LUA_ON_VOTE_END, "iiisi", result, owner, type, text, num1 ) == Lua::PLUGIN_BLOCK ) result = VOTE_NO;
        ...
        if ( !fromLua ) Lua::callHandler( LUA_ON_VOTE_END_AFTER, "iiisi", result, owner, type, text, num1 );
    }
...
bool scallvote(voteinfo *v, ENetPacket *msg) // true if a regular vote was called
{
    ...
    double *var = ( double* ) Lua::getFakeVariable( LUA_ON_PLAYER_CALL_VOTE, NULL, LUA_TNUMBER, "iisii", v->owner, v->type, v->text, v->num1, error );
    if ( var != NULL )
    {
      error = (int) var[0];
...
int getpongflags(enet_uint32 ip)
{
    ...
    bool banned = false;
    loopv(bans) if(bans[i].address.host == ip) { banned = true; break; }
    bool *var = ( bool* ) Lua::getFakeVariable( LUA_YIELD_IS_BANNED, NULL, LUA_TBOOLEAN, "sb", iptoa( ip ), banned );
    if ( var != NULL )
    {
      banned = (bool) var[0];
      delete[] var;
